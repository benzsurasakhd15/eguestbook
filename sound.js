(()=>{
  let ctx,enabled=false,player,playing=false;
  ...
  function start(){if(!enabled||playing)return;player=document.createElement('iframe');player.id='wanwanMusic';player.title='WANWAN background music';player.allow='autoplay';player.src='https://www.youtube.com/embed/GBLcMvTFGyg?autoplay=1&loop=1&playlist=GBLcMvTFGyg&controls=0&playsinline=1';document.body.append(player);playing=true;update()}
  function stop(){player?.remove();player=null;playing=false;update()}
